...
// use this function to read/save info about library elements (XbeLib, etc)
   ...
   std::cout << "get_lib_elements:" << std::endl;
// cout.flush();

   n = count_lines_1(filename);

   inf.open(filename,ios::in|ios::binary);
   if (!inf.is_open()) {
     cout << "get_lib_elements: " << filename << " could not be opened. Halting..." << endl;
     exit(1);
   }

   for (int i=0; i < n; i++) {
//   std::cout << "get_lib_elements: i = " << i << std::endl;
     next_line(inf,v1);
     x = T(v1[0],global);
     xvec.push_back(x);
   }

...
double stod_suffix(const std::string &s1) {
   ...
   if (flag_suffix) {
     x = factor1*stod(s1.substr(0,n-1));
   } else {
     x = stod(s1);
   }
   return x;
...
//     multiply by (s-z) (z = a - jb)
     ...
     cout << "partial_fractions_1: coef1[i]:" << endl;
     for (int i1 = 0;i1 < degree_d;i1++) {
       cout << "  " << i1 << "  " << coef1[i1] << endl;
     }
     for (int j = 0; j < degree_d; j++) {
       A[j][i_pp] = coef1[j];
     }
...
// write A and B to console:
   cout << "partial_fractions_1: A[i][j]:" << endl;
   for (int i = 0;i < pp_n_roots;i++) {
     for (int j = 0;j < pp_n_roots;j++) {
       cout << "  " << i << "  " << j << "  " << A[i][j] << endl;
     }
   }
   cout << "partial_fractions_1: B[i]:" << endl;
   for (int i = 0;i < pp_n_roots;i++) {
     cout << "  " << i << "  " << B[i] << endl;
   }
...
// Solve equations for pp coefficients:
   ...
   cout << "partial_fractions_1: pp_root, pp_power, pp_coef:" << endl;
   for (int i = 0;i < pp_n_roots;i++) {
     cout << "  " << i
      << "  " << pp_root[i]
      << "  " << pp_power[i]
   ...
   cout << "partial_fractions_1: gain=" << gain << ", a0=" << a0 << endl;
   ...
   cout << "partial_fractions_1: ppc_flag_real,ppc_root,ppc_power,ppc_coef:" << endl;

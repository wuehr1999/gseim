...
#           print('_check calling _check_dict')
...
#               print('_check_var_key_dict calling _check_dict')
    ...
    def _check_dict(self, data, scheme):
        ...
        for key, (types_, required, item_scheme) in iter(scheme.items()):
            try:
                value = data[key]
            except KeyError:
                if required:
        ...
        for key in set(data).difference(scheme):
            if key not in ('templates', 'cpp_templates'):

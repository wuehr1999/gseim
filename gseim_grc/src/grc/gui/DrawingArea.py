...
#        self.connect('focus-out-event', self._handle_focus_lost_event)
    ...
    def _handle_mouse_button_press(self, widget, event):
        ...
        if event.type == Gdk.EventType.BUTTON_PRESS \
            and event.button == 1:
            self._flow_graph.click_type = 1
            self._flow_graph.click_left_pos_x = event.x/self.zoom_factor
            self._flow_graph.click_left_pos_y = event.y/self.zoom_factor
        if event.type == Gdk.EventType.BUTTON_PRESS \
            and event.button == 3:
            self._flow_graph.click_type = 3
            self._flow_graph.click_right_pos_x = event.x/self.zoom_factor
            self._flow_graph.click_right_pos_y = event.y/self.zoom_factor
    ...
    def _update_size(self):
        ...
        # See https://github.com/gnuradio/gnuradio/pull/5111/files
        scale_factor = self.get_scale_factor()
        self.set_size_request(w * scale_factor * self.zoom_factor + 100,
    ...
    def draw(self, widget, cr):
        ...
        cr.set_line_width(1.2 / self.zoom_factor)

...
def show_doc_1(platform, key):
    if not key:
        return
    l1 = filter(lambda b: b.key == key, list(platform.blocks.values()))
    block1 = next(l1, None)
    if not block1:
        print('show_doc_1:', key, 'not in blocks or subcircuits? Halting...')
        sys.exit()
    l = block1.loaded_from.rsplit('/', 1)

#   block pdf file: be_xxx.pdf (covers both xbe and ebe)
#   subckt pdf file: sub_xxx.pdf

    if '.hblock' in l[1]:
        pdf_file = l[0] + '/sub_' + l[1].replace('.hblock.yml', '.pdf')
    else:
        pdf_file = l[0] + '/be_' + l[1].replace('.block.yml', '.pdf')
    ...
    for x in l_pdf_readers:
        if cmd_exists(x):
            if (os.path.exists(pdf_file)):
                print('show_doc_1:', pdf_file, 'found.')
                cmd = x + ' ' + pdf_file
                os.system(cmd)
            else:
                print('show_doc_1:', pdf_file, 'not found.')
                s = 'Cannot display document.\n' + pdf_file + ' not found'
                display_message(s)
            break
...
def _format_doc(doc):
    ...
    for block_name, docstring in iter(doc.items()):
...
class BlockTreeWindow(Gtk.VBox):
    ...
    def add_block(self, block, treestore=None, categories=None):
        ...
        treestore.set_value(iter_, NAME_INDEX, block.key)
        treestore.set_value(iter_, DOC_INDEX, '')
    ...
    def _handle_mouse_button_press(self, widget, event):
        ...
        if event.button == 1 and event.type == Gdk.EventType._2BUTTON_PRESS:
            ...
            print('_handle_mouse_button_press: right button clicked')
            key = self._get_selected_block_key()
            print('_handle_mouse_button_press: key:', '<'+key+'>')
            show_doc_1(self.platform, key)
